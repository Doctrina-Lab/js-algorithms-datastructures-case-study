...
describe('Graph.breadthFirst', () => {
  ...
  it('traverses linear graph in breadth-first order', () => {
    let graph = new Graph()
    graph.addVertex('A')
    graph.addVertex('B')
    graph.addVertex('C')
  ...
  it('traverses graph with branches in breadth-first order', () => {
    let graph = new Graph()
    graph.addVertex('A')
    graph.addVertex('B')
    graph.addVertex('C')
    ...
    // A should be first
    expect(result[0]).toEqual('A')
    // B and C should come before D (breadth-first - visit all neighbors at current level)
  ...
  it('traverses complex graph and visits all reachable vertices', () => {
    ...
    // A should be first
    expect(result[0]).toEqual('A')
    // B and C should come before D and E (level 1 before level 2)
    expect(result.indexOf('B')).toBeLessThan(result.indexOf('D'))
  ...
  it('returns empty array when starting from non-existent vertex', () => {
    ...
    graph.addVertex('B')
    graph.addEdge('A', 'B')
    expect(graph.breadthFirst('Z')).toEqual([])
  })

  it('handles null or undefined start vertex', () => {
    let graph = new Graph()
    graph.addVertex('A')
    expect(graph.breadthFirst(null)).toEqual([])
    expect(graph.breadthFirst(undefined)).toEqual([])
  })
})